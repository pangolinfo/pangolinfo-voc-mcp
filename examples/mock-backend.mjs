 ...
 *
 * 演示用的极简 mock 后端 —— 扮演真实链路中 Pangolinfo 后端的
 * `/api/v1/social/*` 那一跳,仅用于本地把 MCP 跑通看数据流。
 *
 * ⚠️ 这不是生产代码。真实后端那一跳还会做:
 *   - 校验 Pangolinfo API Key、从已认证会话取 userId(注入为 externalUserId)
 *   - 扣费(deductBalance,受理即扣)、按操作差异化定价
 *   - 上游错误码 → Pangolinfo 6 类错误模型映射
 * 这里只做最小转发:注入上游 token + 一个演示用 externalUserId → 透传上游。
 *
 * 凭证从环境变量读,绝不硬编码(本仓是 public)。运行前先 export:
 *   UPSTREAM_TOKEN_ENDPOINT  (如 https://<upstream-provider>/oauth/token)
 *   UPSTREAM_API_BASE        (如 https://<upstream-provider>/partner/v1)
 *   UPSTREAM_CLIENT_ID
...
const TOKEN_EP = need("UPSTREAM_TOKEN_ENDPOINT");
const DS_BASE = need("UPSTREAM_API_BASE");
const CLIENT_ID = need("UPSTREAM_CLIENT_ID");
const CLIENT_SECRET = need("UPSTREAM_CLIENT_SECRET");
...
async function getToken() {
  ...
  // 上游要求 client_secret_basic:凭证走 Basic Auth 头。
...
const server = createServer(async (req, res) => {
  // MCP 的 client 调 baseUrl + /api/v1/social/<...>。剥前缀拼到上游。
  ...
  try {
    ...
    const dsRes = await fetch(`${DS_BASE}${dsPath}`, {
      ...
      headers: {
        ...
        // 真实透传用的 external-user-id 请求头名由上游契约定义,此处仅为演示占位。
        "X-External-User-Id": EXTERNAL_USER,
