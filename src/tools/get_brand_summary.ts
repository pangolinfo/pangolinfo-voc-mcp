 ...
 * (扣费、异步、深度自由提问)区分。依赖品牌已有数据 —— 若报 data not ready,先 refresh_brand。
 */
...
const inputSchema = z.object({
  ...
  days: z
    ...
    .optional()
    .describe(
      t({ zh: "摘要覆盖的时间窗(天),可选。", en: "Summary time window in days (optional)." }),
    ),
});
...
与 analyze_brand 区分:analyze_brand 是扣费 + 异步 + 可自由提问的深度报告;本工具是免费 + 同步 + 固定的一段式摘要,可放心调。
...
Vs analyze_brand: analyze_brand is charged + async + free-form deep report; this is free + synchronous + a fixed one-paragraph summary — safe to call freely.
