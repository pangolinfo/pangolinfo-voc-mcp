 ...
 * 上游确认 summary **不扣费**:这是同步、快速的一段式总结,与 report_follow_up_analysis
...
与 report_follow_up_analysis 区分:report_follow_up_analysis 是扣费 + 同步 + 可自由提问的深度报告;本工具是免费 + 同步 + 固定的一段式摘要,可放心调。
依赖品牌已有数据 —— 若报 data not ready,先 refresh_brand 采集。
Returns: data{ summary(一段文本), highlights[] }。
Use when: 用户要"这品牌现在整体怎么样"的快速总览。
Don't use: 要带具体提问的深度分析/选号(用 report_follow_up_analysis,扣费);要结构化指标(用 get_brand_metrics)。`,
...
Vs report_follow_up_analysis: report_follow_up_analysis is charged + synchronous + free-form deep report; this is free + synchronous + a fixed one-paragraph summary — safe to call freely.
...
Don't use: for deep analysis with a specific question / KOC-pick (use report_follow_up_analysis, charged); for structured metrics (use get_brand_metrics).`,
  }),
  inputSchema,
