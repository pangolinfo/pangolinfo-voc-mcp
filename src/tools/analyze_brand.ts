/**
 * Tool: analyze_brand —— 上游固定口径深度分析 shortcut(同步,扣费)。
 *
...
⚠️ 固定口径:analyze 走 上游固定分析链路,**不支持提示词覆盖**。想完全用自己的口吻/结构,改调只读端点(metrics/posts/find_posts_about/sentiment/voice-share/risk-alerts)后用自己的 LLM 输出。
Returns: { report, usage, billing{chargedPoints:600,chargedOn:'success'} }(report 是报告正文)。
Use when: 想要 上游直接给结论的 shortcut。
...
Fixed pipeline: analyze runs the upstream fixed analysis pipeline and does NOT support prompt override. To fully control tone/structure, call the read-only endpoints (metrics/posts/find_posts_about/sentiment/voice-share/risk-alerts) and reason with your own LLM instead.
