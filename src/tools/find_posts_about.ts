...
注意(refreshing 锁):若品牌处于 refreshing/dataReady:false(某平台采集卡住会锁住整个品牌),本工具会被阻塞报错。此时别卡在这、改用聚合读取(get_brand_summary/get_brand_sentiment/get_risk_alerts)或 report_follow_up_analysis 绕过,先出能出的部分。
...
Note (refreshing lock): if the brand is in refreshing / dataReady:false (one platform stuck collecting locks the whole brand), this tool is blocked and errors out. Don't get stuck — fall back to aggregate reads (get_brand_summary/get_brand_sentiment/get_risk_alerts) or report_follow_up_analysis to produce what you can.
