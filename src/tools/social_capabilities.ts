  ...
  async execute(_input, ctx) {
    ...
    return {
      ...
      workflows: [
        ...
        t({
          zh: "产出 VOC 报告(重要):数据就绪后,尽量把能拿的真实数据都拉上,让报告详尽、可追溯、值回积点。可用数据源(各给不同维度,报告里标明来自哪个工具):get_brand_metrics(总量+分平台+正负驱动词 TOP 带次数)、get_brand_sentiment(整体/分平台情感)、get_risk_alerts(高风险负面评论原文+情感分/置信度/作者/帖子/链接,窗口常为 72h)、find_posts_about/search_brand_posts(真实帖子原话+作者+平台+真实点赞/播放)、get_voice_share(竞品声量,仅配了竞品时有)、get_brand_summary(AI 摘要作叙事骨架)。排版结构由你自己定。**铁律:数字/原话/互动数一律用工具真实返回值,严禁编造/估算/补全;拿不到就标「未返回」,别填假值。**",
          en: "Produce a VOC report (important): once data is ready, pull as much real data as you can obtain to make the report thorough, traceable, and worth the points. Available data sources (each a different dimension; label which tool each block came from): get_brand_metrics (totals + per-platform + positive/negative driver words TOP with counts), get_brand_sentiment (overall/per-platform sentiment), get_risk_alerts (high-risk negative comment quotes + sentiment score/confidence/author/post/URL, window usually 72h), find_posts_about/search_brand_posts (real post quotes + author + platform + real likes/views), get_voice_share (competitor share of voice, only when competitors configured), get_brand_summary (AI summary as narrative skeleton). You decide the layout/structure. **Hard rule: every number/quote/engagement figure must be the tool's real returned value — never fabricate/estimate/fill in; if not returned, mark 'not returned', don't invent a value.**",
        }),
      ],
      notes: [
        t({
